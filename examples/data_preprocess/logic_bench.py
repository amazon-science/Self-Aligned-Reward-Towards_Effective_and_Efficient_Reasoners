...
if __name__ == "__main__":
    ...
    dataset_list = []
    path = "LogicBench/data/LogicBench(Eval)/MCQA"
        ...
        for filename in filenames:
            if filename.endswith(".json"):
                ...
                with open(file_path, 'r') as f:
                    ...
                    dataset_list.append(Dataset.from_list(raw_data))
    ...
    data_source = "LogicBench"
    def make_map_fn(split):
        def process_fn(example, idx):

            question = example.pop("context") + "\n" + example.pop("question") + " Choose from the following answers:\n" + '\n'.join([chr(ord('A') + id) + ': ' + x for id, x in enumerate(example.pop("choices").values()) if isinstance(x, str)]) + '\n' + instruction_following
            answer = chr(ord('A') + (int)(example.pop("answer")[-1]) - 1)
    ...
    # Create training dataset from last 80% of original data
    ...
    for dirpath, dirnames, filenames in os.walk(path):
        for filename in filenames:
            if filename.endswith(".json"):
                ...
                with open(file_path, 'r') as f:
                    raw_data = json.load(f)["samples"]
                    original_dataset_list.append(Dataset.from_list(raw_data))
    
    original_dataset = datasets.concatenate_datasets(original_dataset_list)
