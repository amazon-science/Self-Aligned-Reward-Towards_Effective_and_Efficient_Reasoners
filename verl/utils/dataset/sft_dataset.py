...
class SFTDataset(Dataset):
    ...
    def __init__(self, parquet_files: Union[str, List[str]], tokenizer, config, proportion=1):
    ...
    def _read_files_and_tokenize(self):
        ...
        for key in self.response_dict_keys:
            try:
                self.responses = self.responses.apply(lambda x: series_to_item(x), axis=1)  # noqa: B023
            except Exception:
    ...
    def __getitem__(self, item):
        ...
        if self.tokenizer.chat_template:
            ...
            prompt_chat_str = f"System: {SYS_PROMPT}\nUser: {prompt}\nAssistant: "
        ...
        # together_str = prompt_chat_str + response_chat_str
        # print(f"together_str: {together_str}")
        # together_ids = tokenizer(together_str, return_tensors="pt", add_special_tokens=False)["input_ids"][0]
        # print(f"together_ids: {together_ids}")
        # assert False
        ...
        if self.config.get("loss_mask", True):
            if prompt_length > 1:
