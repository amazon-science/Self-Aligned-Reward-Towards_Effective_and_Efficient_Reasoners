...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    Returns:
        A dictionary of metrics including:
            ...
            - rewards/overall: the overall reward for the batch, same as critic/rewards/mean
            - rewards/xxx, where xxx is the name of the reward function: statistics about each reward function
    """
    ...
    metrics = {
        ...
        **(
        ...
        # rewards calc
        "rewards/overall": torch.mean(sequence_reward).detach().item(),
        **(
            {f"rewards/{k}": torch.mean(v).detach().item() for k, v in batch.batch.items() if k.startswith("rewards/") and isinstance(v, torch.Tensor) and v.ndim == 1}
        )
    }
