...
class FSDPSFTTrainer:
    ...
    def _build_model_optimizer(self):
        ...
        with init_context():
            ...
            if type(self.model).__name__ == "Gemma3ForConditionalGeneration":
    ...
    def save_checkpoint(self, step, final=False):
        ...
        if self.device_mesh.get_rank() == 0:
            if final:
                save_model_path = os.path.join(self.config.trainer.default_local_dir, f"final_model")
            else:
                save_model_path = os.path.join(self.config.trainer.default_local_dir, f"global_step_{step}")

            os.makedirs(save_model_path, exist_ok=True)
            self.model.save_pretrained(save_model_path, state_dict=state_dict)
            self.tokenizer.save_pretrained(save_model_path)

            if self.config.trainer.default_hdfs_dir:
                hdfs_io.makedirs(self.config.trainer.default_hdfs_dir, exist_ok=True)
                hdfs_io.copy(src=save_model_path, dst=self.config.trainer.default_hdfs_dir, dirs_exist_ok=True)
    ...
    def fit(self):
        ...
        # Perform initial validation before training begins
        if rank == 0:
            print("Performing initial validation (step 0)...")
        val_losses = []
        ...
        if rank == 0:
            ...
            print(f"Initial validation loss: {avg_val_loss.detach().item()}")
        torch.distributed.barrier()
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            # save checkpoint
            if self.config.trainer.save_intermediate:
...
def main(config):
    ...
    val_dataset = create_sft_dataset(config.data.val_files, config.data, tokenizer, val=True)
...
def create_sft_dataset(data_paths, data_config, tokenizer, val=False):
    """Create a dataset."""
    # build dataset
    ...
    dataset = dataset_cls(parquet_files=data_paths, tokenizer=tokenizer, config=data_config, proportion=data_config.val_proportion if val else data_config.train_proportion)
    # print(dataset[0]) # debug
    return dataset
