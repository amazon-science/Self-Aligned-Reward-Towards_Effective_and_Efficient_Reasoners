...
# GLOO settings
export GLOO_SOCKET_IFNAME=$(ip route get 8.8.8.8 | awk '{print $5; exit}')
...
echo "Using GLOO backend on interface: $GLOO_SOCKET_IFNAME"
...
python -c "import gloo_patch" && python3 -m verl.trainer.main_ppo \
    ...
    trainer.reward_factors=[1.0,0.2] \
